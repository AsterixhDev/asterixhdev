...
export default function Header() {
  ...
  return (
    <header
      ...
      className="sticky w-[95vw] sm:w-[90vw]  top-2 rounded-full mx-auto right-0 z-50 bg-black/60 backdrop-blur-lg border-b border-gray-800"
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-4">
        {/* Logo / Brand Name */}
        <div className="text-base sm:text-2xl font-extrabold text-white tracking-wide">
          <WordRotate
            words={["AsterixhDev", "Web developer"]}
            duration={5000}
          />
        </div>
        {/* Navigation Menu */}
            ...
            {(Object.keys(links) as Array<keyof typeof links>).map((link) => (
              <ScrollToNavigator
                ...
                className={clsx(
                  ...
                  currentPath === links[link] &&
                    "before:!w-1/2 before:!opacity-100"
                )}
              >
                {link}
              </ScrollToNavigator>
        ...
        <div className="flex items-center gap-3">
          <ScrollToNavigator href={"/#contact"}>
